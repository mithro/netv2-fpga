...
class FrameExtraction(Module, AutoCSR):
    def __init__(self, word_width, fifo_depth, mode="ycbcr422"):
        ...
        if mode == "ycbcr422":
            # Imported lazily: the ycbcr422 colour-space conversion pulls in the
            # litevideo csc cores, which the input0 (lock + TERC4 only) path never
            # instantiates.  Keeping the import here lets analysis.py load without
            # the csc package present.
