...
class _Slot(Module, AutoCSR):
    def __init__(self, addr_bits, alignment_bits):
        ...
        # Modern LiteX (2026.04) dropped CSRStorage(alignment_bits=...).  The
        # firmware still programs *byte* addresses, so keep the CSR full byte
        # width and derive the word address by dropping the low alignment_bits
        # here rather than in the CSR primitive.
        ...
        self.comb += [
            ...
            self._address.dat_w.eq(self.address_reached << alignment_bits),
...
class DMA(Module):
    def __init__(self, dram_port, nslots):
        bus_aw = dram_port.address_width
        bus_dw = dram_port.data_width
        alignment_bits = bits_for(bus_dw//8) - 1

        fifo_word_width = bus_dw
        self.frame = stream.Endpoint([("sof", 1), ("pixels", fifo_word_width)])
        # Byte-sized frame CSR (see _Slot): word count is storage >> alignment_bits.
